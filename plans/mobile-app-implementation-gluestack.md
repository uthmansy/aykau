# Aykau Mobile App Implementation Plan (gluestack-ui)
...
Build a native mobile application for Aykau using React Native, Expo, and gluestack-ui that mirrors the web platform's functionality while providing a native mobile experience.

**Tech Stack:**

- **React Native** - Cross-platform mobile framework
- **Expo** - Development platform and tooling
- **gluestack-ui** - UI component library with design system
...
# Install gluestack-ui and dependencies
...
│
├── theme/                        # gluestack-ui theme
│   ├── config.ts                 # gluestack-ui configuration
│   ├── tokens.ts                 # Design tokens
│   ├── themes.ts                 # Light/dark themes
│   └── components/               # Custom component themes
│       ├── Button.ts
│       ├── Card.ts
│       ├── Input.ts
│       └── index.ts
...
├── gluestack.config.ts           # gluestack-ui config
...
## 3. gluestack-ui Theme Configuration
...
export const Button = createButton({
  baseStyle: {
    _text: {
      fontWeight: config.tokens.fontWeights.semibold,
    },
    borderRadius: config.tokens.radii.default,
    px: config.tokens.space.md,
    py: config.tokens.space.sm,
  },
  variants: {
    solid: {
      bg: config.tokens.colors.primary,
      _text: {
        color: config.tokens.colors.onPrimary,
      },
      _pressed: {
        bg: config.tokens.colors.primaryContainer,
      },
    },
    outline: {
      borderWidth: 1,
      borderColor: config.tokens.colors.outline,
      _text: {
        color: config.tokens.colors.onSurface,
      },
    },
    ghost: {
      _text: {
        color: config.tokens.colors.primary,
      },
    },
  },
  sizes: {
    sm: {
      _text: {
        fontSize: config.tokens.fontSizes.labelSm,
      },
      px: config.tokens.space.sm,
      py: config.tokens.space.xs,
    },
    md: {
      _text: {
        fontSize: config.tokens.fontSizes.labelMd,
      },
    },
    lg: {
      _text: {
        fontSize: config.tokens.fontSizes.bodyMd,
      },
      px: config.tokens.space.lg,
      py: config.tokens.space.md,
    },
  },
});
```
...
import { createCard } from "@gluestack-ui/themed";
import { config } from "../../gluestack.config";

export const Card = createCard({
  baseStyle: {
    bg: config.tokens.colors.surfaceContainerLow,
    borderRadius: config.tokens.radii.lg,
    padding: config.tokens.space.md,
    shadow: config.tokens.shadows.level2,
  },
  variants: {
    elevated: {
      shadow: config.tokens.shadows.level2,
    ...
    outlined: {
      borderWidth: 1,
      borderColor: config.tokens.colors.outlineVariant,
    },
    glass: {
      bg: config.tokens.glass.background,
      borderWidth: 1,
      borderColor: config.tokens.glass.border,
    },
  },
});
```
...
import { createInput } from "@gluestack-ui/themed";
import { config } from "../../gluestack.config";
...
export const Input = createInput({
  baseStyle: {
    ...
    _placeholder: {
      color: config.tokens.colors.onSurfaceVariant,
    },
    _focus: {
    ...
    _invalid: {
      borderColor: config.tokens.colors.error,
    },
  },
});
```
...
export const AuthInput = ({
  ...
  return (
    <VStack space="xs">
      <Text fontSize="labelMd" fontWeight="medium" color="$onSurface">
        {label}
      </Text>
      <Input variant={error ? "error" : "outline"}>
        {icon && (
          <InputSlot>
      ...
      {error && (
        <Text fontSize="labelSm" color="$error">
          {error}
        </Text>
      )}
    </VStack>
...
export const RoleSelector = ({ value, onChange }: RoleSelectorProps) => {
  return (
    <HStack space="sm">
      <Button
        flex={1}
        variant={value === "customer" ? "solid" : "outline"}
        onPress={() => onChange("customer")}
      >
        <ButtonText>I need services</ButtonText>
      </Button>
      <Button
        flex={1}
        variant={value === "artisan" ? "solid" : "outline"}
        onPress={() => onChange("artisan")}
      >
        <ButtonText>I offer services</ButtonText>
      </Button>
    </HStack>
  );
};
...
export const JobCard = ({ job, onPress, showDistance }: JobCardProps) => {
  return (
    <Pressable onPress={onPress}>
      <Card variant="elevated">
        <Text fontSize="bodyLg" fontWeight="semibold" color="$onSurface">
          {job.title}
        </Text>
        <Text fontSize="labelMd" color="$onSurfaceVariant">
          {job.category}
        </Text>
        <Text fontSize="bodyMd" color="$onSurface">
          {job.description}
        </Text>
        {showDistance && job.distance && (
          <Text fontSize="labelSm" color="$onSurfaceVariant">
...
export const MessageBubble = ({ message, isOwn, showAvatar }: MessageBubbleProps) => {
  return (
    <Card
      variant="elevated"
      bg={isOwn ? "$primary" : "$surfaceContainerHigh"}
      alignSelf={isOwn ? "flex-end" : "flex-start"}
      maxWidth="80%"
    >
      <Text color={isOwn ? "$onPrimary" : "$onSurface"}>
        {message.content}
      </Text>
      <Text
        fontSize="labelSm"
        color={isOwn ? "$onPrimary" : "$onSurfaceVariant"}
        alignSelf="flex-end"
      >
        {message.created_at}
      </Text>
  ...
  return (
    <Card variant="glass">
      <VStack space="md">
        ...
        <Text fontSize="displayLg" fontWeight="bold" color="$onSurface">
          ₦{fiatBalance.toLocaleString()}
        </Text>
        <Text fontSize="bodyMd" color="$onSurfaceVariant">
          {creditBalance} credits
        </Text>
        <HStack space="sm">
          <Button flex={1} onPress={onBuyCredits}>
            <ButtonText>Buy Credits</ButtonText>
          </Button>
          <Button flex={1} variant="outline" onPress={onWithdraw}>
...
const OnboardingFlow = () => {
  ...
  const steps = [
    { component: WelcomeScreen, next: () => setStep(1) },
    {
      ...
      next: (selectedRole: "customer" | "artisan") => {
...
| UI Library    | Ant Design + Tailwind | gluestack-ui               |
...
## 15. gluestack-ui vs Tamagui Comparison

| Feature            | Tamagui                   | gluestack-ui                       |
| ------------------ | ------------------------- | ---------------------------------- |
| Component Model    | Styled system with tokens | Headless + Themed components       |
| Customization      | createTamagui config      | createConfig + component overrides |
| Design Tokens      | Built-in token system     | Token-based theming                |
| Accessibility      | Good                      | Excellent (WAI-ARIA compliant)     |
| Bundle Size        | Larger                    | Smaller (tree-shakeable)           |
| Learning Curve     | Moderate                  | Low (familiar React patterns)      |
| Community          | Growing                   | Established (NativeBase legacy)    |
| TypeScript Support | Excellent                 | Excellent                          |
| Animation          | Built-in animations       | Reanimated integration             |

---

## Summary

This plan provides a comprehensive roadmap for building the Aykau mobile app using React Native, Expo, and gluestack-ui. The architecture mirrors the web app's structure while leveraging native mobile capabilities for a superior user experience.
...
- **Accessible UI** with gluestack-ui (WAI-ARIA compliant)
...
gluestack-ui provides excellent accessibility out-of-the-box and a familiar component model that will accelerate development while maintaining design system compliance with the Luminous Marketplace specifications.
